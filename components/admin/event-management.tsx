...
import { Plus, Edit, Trash2, Loader2, Calendar, User, Megaphone, X } from "lucide-react";
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <Field data-invalid={fieldState.invalid}>
                      ...
                      <Input {...field} id={field.name} placeholder="Event Title" />
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <Field data-invalid={fieldState.invalid}>
                      ...
                      <Input {...field} id={field.name} placeholder="event-slug" />
                      {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                    </Field>
                  )}
                />
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <Field data-invalid={fieldState.invalid}>
                      ...
                      <Input {...field} type="datetime-local" id={field.name} />
                      {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                    </Field>
                  )}
                />
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    ...
                    <Textarea {...field} id={field.name} placeholder="Tell us more about the event..." />
                    {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                  </Field>
                )}
              />
              ...
              <div className="space-y-4 pt-4">
                <div className="flex justify-between items-center">
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={() => append({ name: "", role: "", image: "" })}>
                    <Plus className="h-4 w-4 mr-2" /> Add Minister
                  </Button>
                </div>
                <div className="space-y-4">
                  {fields.map((field, index) => (
                    <div key={field.id} className="grid grid-cols-12 gap-2 items-start border p-3 rounded-md relative group">
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="hidden md:table-cell text-center">Ministers</TableHead>
                ...
                <TableRow key={event.id}>
                  ...
                  <TableCell className="font-medium">
                    <div className="flex items-center gap-3">
                      {event.poster ? (
                        <img src={event.poster} alt={event.title} className="h-10 w-10 rounded object-cover" />
                      ) : (
                      ...
                      <div>
                        {event.title}
                        <div className="text-xs text-muted-foreground font-normal">/{event.slug}</div>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="hidden md:table-cell">
                    <div className="flex justify-center -space-x-2">
                      {event.ministers.slice(0, 3).map((m, i) => (
                        <div key={i} className="h-8 w-8 rounded-full border-2 border-background overflow-hidden bg-muted" title={m.name}>
                          {m.image ? <img src={m.image} alt={m.name} className="h-full w-full object-cover" /> : <User className="h-4 w-4 m-1.5 text-muted-foreground" />}
                        </div>
                      ))}
                  ...
                  <TableCell className="text-right text-muted-foreground">
                    <div className="flex justify-end gap-2">
                      ...
                      <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive" onClick={() => onDelete(event.id)}>
